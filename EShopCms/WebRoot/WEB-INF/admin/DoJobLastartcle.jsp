...
<!--文章类型审批审核任务处理-->
  <form action="doNewsPassType.jspx" method="post">
   <input type="hidden" name="jobId" value="<s:property value="getJobId()"/>"/>
  <input type="hidden" name="artId" value="<s:property value="#news.getArticleId()"/>"/>
<table  border="00" cellspacing="1" cellpadding="0" id="table" class="sortable">
 <thead> 
     ...
     <tbody>
      <tr>
        ...
        <td ><s:property value="#news.getTitle()"/></td>
          ...
          <tr>
        <td  width="15%" >发布时间：</td>
        <td ><s:property value="#news.updateTime"/></td>
      </tr>
          <tr>
        <td  width="15%" >内容：</td>
        <td ><s:set name="content" value="#news.getContent()"/>
        <s:property escape="false"  value="cutStrContent(#content)"/>        </td>
      </tr>
          <tr>
        <td  width="15%" >目前状态：</td>
        <td ><s:set name="status" value="#news.getStatus()"/>
        <s:if test="#status==0">
        未审核        </s:if>
         <s:if test="#status==1">
        一审完成        </s:if>
