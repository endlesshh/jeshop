...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 minwidth: 30, //每列最小宽度
			 minheight: 100, //最小高度
			 resizable: false, //改变table的大小。
			 url: false, //ajax url
			 method: 'POST', 
			 dataType: 'xml', // 默认是 xml 
			 errormsg: '连接错误！',
			 usepager: true, //
			 nowrap: true, //
			 page: 1, //默认第一页
			 total: 1, //默认总共一页
			 useRp: true, //是否显示下拉框（每页显示多少的那个下拉框）。
			 rp: 15, // 默认15条
			 rpOptions: [10,15,20,25,40],// 设置下拉框的选项
			 title: false, //标题是否显示
			 pagestat: '显示 第{from}条 到 第{to}条 , 总共 {total}条记录.',
			 procmsg: '加载中, 请稍等 ...',
			 query: '', //查询条件默认是空
			 qtype: '',
			 nomsg: '无数据！',
			 minColToggle: 1, //至少显示 1列
			 showToggleBtn: true, //是否隐藏  控制显示和隐藏的  按钮 . 这个不需要改。暂时有错误
			 hideOnSubmit: true,
			 autoload: true, //自动加载第一页
			 blockOpacity: 0.5, //透明度
			 onToggleCol: true, //是否可以拖动列
			 onChangeSort: false,//是否可以改变排序。这个不需要改。暂时有错误
			 onSuccess: false,//这个不需要改。暂时有错误 .
			 onSubmit: false // 这个不需要改。暂时有错误 .using a custom populate function
		  }, p);
		...
		var g = {
			...
			doSearch: function () {
				p.query = encodeURIComponent($('input[name=q]',g.sDiv).val()); //对查询的中文字 进行编码
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>搜索 : <input type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select> <input type='button' value='显示全部数据' /></div>");

				$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13) g.doSearch()});
				$('input[value=显示全部数据]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val(''); p.query = ''; g.doSearch(); });
				$(g.bDiv).after(g.sDiv);				
