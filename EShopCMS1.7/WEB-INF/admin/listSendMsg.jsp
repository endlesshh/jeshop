...
<thread><tr>
  ...
  <th width="5%">消息状态</th><th width="11%">消息收件人</th><th width="24%">消息内容</th><th width="18%">发送时间</th><th>阅读时间</th></tr></thread>

  <s:set name="pagebean" value="pageReListSendMsg()"/>
<s:iterator value="pageReListSendMsg().getList()">
<tr>
  <td>
  <input type="checkbox" name="deleID" value="<s:property  value="getId()"/>">  </td>
  <td>
<s:if test="getStat()==0">未读

<a href="delSubMsg.jspx?id=<s:property value="getId()"/>&linkUrl=listSendMsg.jsp">
删
</a>
</s:if>
...
<a onClick="window.showModalDialog('UserDetail.jspx?UserId=<s:property value="#tempuser.getUserId()"/>',window,'status:no;scroll:no;dialogWidth:600px;dialogHeight:650px');return false;" href="UserDetail.jspx?UserId=<s:property value="#tempuser.getUserId()"/>">
<s:property value="#tempuser.getUserName()"/></a>
</s:else>
...
<td title="header=[<s:property value="getsDate()"/>] body=[<s:property  value="getMsg()"/>]">&nbsp;查阅内容
...
<a onClick="return confirm('是否确定删除！删除后不可恢复')" href="delOverMsg.jspx?id=<s:property value="getId()"/>&linkUrl=listSendMsg.jsp">
彻底删除 </a> <a onClick="return confirm('是否确定删除！删除后不可恢复')" href="delOverMsg.jspx?id=<s:property value="getId()"/>&linkUrl=listSendMsg.jsp">
回复消息 </a>
-->
&nbsp;<s:property value="getrDate()"/>
</td>
</tr>
</s:iterator>
</table>
...
<div align="center">
   ...
   <s:url id="ListNews" value="/admin/listSendMsg.jspx">
  <s:param name="page"><s:property value="%{#pagebean.currentPage+1}"/></s:param>
        ...
        <s:if test="%{#pagebean.currentPage==1}">
            第一页 上一页        </s:if>
        <s:else>
            <s:url id="firstNews" value="/admin/listSendMsg.jspx">
           <s:param name="page">1</s:param>
           </s:url>
            <s:a href="%{firstNews}">第一页</s:a>
              <s:url id="preNews" value="/admin/listSendMsg.jspx">
           <s:param name="page"><s:property value="%{#pagebean.currentPage-1}"/></s:param>
           </s:url>
