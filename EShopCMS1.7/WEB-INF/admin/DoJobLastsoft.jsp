...
<body>
   ...
   <s:set name="soft"  value="listObject"/>
<table  border="00" cellspacing="1" cellpadding="0" id="table" class="sortable">
 <thead> 
      
      <tr>
        <th colspan="6" class="nosort" ><s:property value="#soft.getSoftName()"/> (历史记录)</th>
      </tr>
  </thead> 
  <tr>
        <td >
...
<!--文章类型审批审核任务处理-->
  <form action="doSoftPassType.jspx" method="post">
   <input type="hidden" name="jobId" value="<s:property value="getJobId()"/>"/>
  <input type="hidden" name="softId" value="<s:property value="#soft.getSoftId()"/>"/>
<table  border="00" cellspacing="1" cellpadding="0" id="table" class="sortable">
 <thead> 
      
      <tr>
        <th colspan="2" class="nosort" ><s:property value="#soft.getSoftName()"/>  (审核)</th>
      </tr>
  </thead>
     ...
     <tbody>
      <tr>
        <td  width="15%" >软件名称：</td>
        <td ><s:property value="#soft.getSoftName()"/>&nbsp; </td>
      </tr>
          <tr>
        <td  width="15%" >发布时间：</td>
        <td ><s:property value="#soft.updateTime"/>&nbsp;</td>
      </tr>
          <tr>
        <td  width="15%" >软件介绍：</td>
        <td ><s:property value="#soft.getSoftIntro()"/>&nbsp;
          </td>
      </tr>
          <tr>
        <td  width="15%" >目前状态：</td>
        <td ><s:set name="status" value="#soft.getStatus()"/>
        ...
        二审完成        </s:if>      &nbsp;  </td>
          ...
          <tr>
        <td  width="15%" >演示地址：</td>
        <td ><s:property value="#soft.getDemoUrl()"/></td>
      </tr>
