...
<table  border="0" width="550" style=" margin-top:40px;" align="center" cellpadding="0" cellspacing="3" >
  <form>
	...
	{
	if (request.getParameter("dbType").equals("mssql")){
	//测试
	...
	session.setAttribute("hostPort",request.getParameter("hostPort"));
	session.setAttribute("dbType",request.getParameter("dbType"));
	...
	if (request.getParameter("dbType").equals("mysql")){
	...
	if(request.getParameter("isNewData")==null)
	{
	//jdbc:mysql://localhost:3918/eshopdb?characterEncoding=utf-8
	   url="jdbc:mysql://"+request.getParameter("hostName")+":"+request.getParameter("hostPort")+"/"+request.getParameter("dbName")+"?characterEncoding=utf-8";  
	   out.print("不创建数据库");
	Connection   conn=DriverManager.getConnection(url,request.getParameter("userName"),request.getParameter("passWord"));    
	if(conn!=null)
	{
  ...
  <tr>
	...
	{
	%>
	<input type="text" name="hostPort" size="4" value="<%=session.getAttribute("hostPort")%>" />
	<%
	}else
	{
	%>
  ...
  <tr>
    ...
    <td align="left">
    <%
	if (session.getAttribute("dbType")!=null)
	{
	%>
	 <select  name="dbType"><option <% if (session.getAttribute("dbType").equals("mssql")){out.print("selected='selected'");}%>  value="mssql">SQL Server</option>
    
    <option <% if (session.getAttribute("dbType").equals("mysql")){out.print("selected='selected'");}%>  value="mysql">My sql</option></select>   
	
	<%
	}else
	{
	%>
	 <select  name="dbType"><option value="mssql">SQL Server</option>
    
    <option  value="mysql">My sql</option></select>   
  ...
  <tr>
	...
	{
	%>
	<input type="text" size="10" name="userName" value="<%=session.getAttribute("userName")%>" />
	<%
	}else
	{
	%>
  ...
  <tr>
	...
	{
	%>
    <input type="text" size="10" name="passWord" value="<%=session.getAttribute("passWord")%>"/>
  ...
  <tr>
	...
	{
	%>
	<input type="text" size="10" name="dbName" value="<%=session.getAttribute("dbName")%>"/>
	<%
	}else
	{
	%>
  ...
  <tr>
	...
	{
	%>
	<input name="按钮" type="button"  value="下一步操作" onclick="if(confirm('执行该操作以后，如果存在数据库数据将被清空，不存在则继续创建数据库！是否继续执行')){window.location.href='ShowSetupInfo.jsp'}else{return false;}" />
