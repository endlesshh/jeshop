...
$.fn.ProvinceCity = function(){
	var _self = this;
	//定义3个默认值
	_self.data("pri",["请选择", "请选择"]);
	_self.data("city",["请选择", "请选择"]);
	_self.data("xian",["请选择", "请选择"]);
	//插入3个空的下拉框
	...
	//默认省级下拉
	if(_self.data("pri")){
		$sel1.append("<option value='"+_self.data("pri")[1]+"'>"+_self.data("pri")[0]+"</option>");
	}
	...
	//默认的2级城市下拉
	if(_self.data("xian")){
		$sel3.append("<option value='"+_self.data("xian")[1]+"'>"+_self.data("xian")[0]+"</option>");
	}
	//省级联动 控制
	var index1 = "" ;
