...
<table  border="0" cellpadding="0" cellspacing="0" id="table" class="sortable">
  ...
  <tr>
    ...
    <tbody>
   <!---审核信息咨询-->
    <s:iterator value="plistsoft" status="soft">
  <tr>
    ...
    <td align="center" valign="middle" >
     <s:set name="status" value="%{getStatus()}"/>
          ...
          <option>审核操作</option>
          <option value="setSoftPass.jspx?softId=<s:property  value="%{getSoftId()}"/>&passValue=1&channelId=<s:property value="getChannelId()"/>">通过一审</option>
          <option value="setSoftPass.jspx?softId=<s:property  value="%{getSoftId()}"/>&passValue=2&channelId=<s:property value="getChannelId()"/>">通过二审</option>
          </select>
          </s:if>
          <s:if test="status==1">
          ...
          <s:if test="status==2">
          <font color="#00CC00">二审通过</font>          </s:if>    </td>
    <td align="center" valign="middle" ><a href="extendSoft.jspx?softId=<s:property value="getSoftId()"/>&channelId=<s:property value="getChannelId()"/>">自定义字段</a> <a href="findSoft.jspx?channelId=<s:property value="getChannelId()"/>&softId=<s:property value="getSoftId()"/>">修改</a> <a onClick="return confirm('是否确认删除本软件信息')" href="deleteSoft.jspx?softId=<s:property value="getSoftId()"/>" >删除</a></td>
  </tr>
  </s:iterator>
  </tbody>
</table>

<div align="center">  全选 <input onClick="selectAllCheck(this.id,'deleID')"  type="checkbox" id="seleall"/>反选 
        <input onClick="selectreCheck(this.id,'deleID')"  type="checkbox" id="reseleall"/>
          <select name="passtypes" onChange="$('#suball').submit()">
          <option>审核操作</option>
